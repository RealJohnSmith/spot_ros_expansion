...
from registry import msgify
from point_cloud2 import *



# unset PYTHONPATH; source /opt/ros/melodic/setup.bash; source ${HOME}/spot_ws/devel/setup.bash
# python3 local_grid.py
#
# terrain_local_grid_corner
#
...
class LocalGridPublisher:
    ...
    def init_ros(self):
        ...
        for grid_name in self.grid_names:
            self.gc_publisher_dict[grid_name] = rospy.Publisher(f"spot/grid/{grid_name}", PointCloud2, queue_size=1)
    ...
    def get_local_grids(self, names):
        ...
        for name, grid in zip(names, grids):
            ...
            # Decode the data
            data = np.zeros((extent.num_cells_x * extent.num_cells_y), dtype=[
                ('x', np.float32),
                ('y', np.float32),
                ('z', np.float32)
            ])

            data_pc = np.zeros((data.shape[0], 3))

            for idx, d in enumerate(data_dec):
                # Height of cell
            ...
            data_pc = grid_to_body_tf.transform_cloud(data_pc)

            data['x'] = data_pc[:, 0]
            data['y'] = data_pc[:, 1]
            data['z'] = data_pc[:, 2]

            data_msg = msgify(PointCloud2, data)
            data_msg.header.frame_id = "body"
            data_msg.header.stamp = rospy.Time.now() # TODO use acquisition_time # rospy.Time(acquisition_time.seconds, acquisition_time.nanos)


            grid_msgs[name] = data_msg
